...
def generate_report(quant_analysis_result: Dict[str, Any], market_sentiment_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        response = _report_agent.model.invoke(
            _report_agent.report_prompt.format(analysis_data=json.dumps(combined_data, ensure_ascii=False, indent=2))
        )
        
        # Parse output
        report = _report_agent.output_parser.parse(response.content)
